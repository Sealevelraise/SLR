<template>
  <div id="app">
    ...
    <button
      class="w-11/12 md:w-2/5 text-slr-blue border-solid border-2 border-slr-blue bg-slr-page-bg duration-500 py-2 px-6 rounded-md"
      v-if="!connected"
      @click="connect"
    >
      Connect wallet
  ...
  methods: {
    ...
    createNewProject: async function (projectName, country, amount) {
      ...
      this.contractResult = await contract.methods
        .addProject(
          this.addProject_Name,
          this.addProject_Country,
          parseInt(this.addProject_Amount)
        )
